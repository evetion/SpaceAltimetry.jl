...
- **Search**: `search` queries NASA Common Metadata Repository (CMR) by mission, product, temporal range, and other metadata.
...
The intended use case is any Julia workflow using NASA satellite laser altimetry data.
SpaceLiDAR is especially adept for repeated regional or global processing from local archives, with outputs remaining in Julia table and geospatial interfaces
...
`SpaceAltimetry.jl` was first presented at JuliaCon 2021 [@pronkmspacelidarjlprocessingicesat22021].
...
It has since been used for glacier modelling [@gardnerSomethingSomethingSpaceAltimetryjl2027].
