...
affiliations:
 - name: Deltares, Delft, The Netherlands
   index: 1
   ror: 01deh9c76
 - name: Jet Propulsion Laboratory, California Institute of Technology, Pasadena, USA
   index: 2
   ror: 027k65916
date: 26 June 2026
...
`SpaceAltimetry.jl` is a Julia [@bezansonJuliaFreshApproach2017] package for searching, downloading, and reading spaceborne lidar data from NASA's ICESat, ICESat-2, and GEDI missions.
It uses mission-specific granule types and product schemas to expose selected HDF5 variables as lazy [Tables.jl][@jacobquinnJuliaDataTablesjlV11302026]-compatible tables that integrate with the Julia ecosystem.

# Statement of need

The lidar satellite altimetry missions by NASA, ICESat (2003-2010) [@schutzOverviewICESatMission2005], ICESat-2 (2019-) [@markusIceCloudLand2017] and GEDI (2019-) [@dubayahGlobalEcosystemDynamics2020] are of great use to the scientific community as primary sources of high precision surface height and vegetation characteristics, among others [@magruderMonitoringEarthsClimate2024].
...
`SpaceAltimetry.jl` fills that gap, allowing satellite altimetry data to be easily passed directly into Julia's geospatial and numerical workflows.

The package implements the following functionality:

- **Search**: `search` queries NASA's Common Metadata Repository (CMR) by mission, product, temporal range, and other metadata.
- **Download and sync**: `download` handles authenticated Earthdata downloads for individual granules. For larger jobs, bundled [`aria2c`](https://aria2.github.io/) support enables parallel downloads, while `sync` maintains a local folder of granules by scanning existing files and downloading only what is missing.
...
- **Filtering and transformations**: Spatial and quality filtering, corrections, and geoid transformations can be chained with Julia's pipe syntax and are specialised to each product. Each operation declares the columns it needs, allowing `SpaceAltimetry.jl` to read the required HDF5 datasets once before materialisation.

The package is intended for any Julia workflow using NASA satellite laser altimetry data.
`SpaceAltimetry.jl` is especially well suited to repeated regional or global processing from local archives, while keeping outputs compatible with Julia's tabular and geospatial interfaces.
...
`SpaceAltimetry.jl` differs by providing a Julia-native interface and by using the same granule, track-selection, table, and operation concepts across ICESat, ICESat-2, and GEDI. This makes mixed-mission workflows possible without separate mission-specific readers in the user's analysis code, something the Python- and R-based tools above cannot offer Julia users.

# Software design

`SpaceAltimetry.jl` is designed around mission abstractions combined with a geospatially aware but generic HDF5-to-table reader. Scientific, mission-specific code remains in product schemas and operations, while the HDF5 table reader is reusable for arbitrary HDF5 files and provides integration with the Julia ecosystem. This design choice was made because early, hard-coded schemas limited extensibility and adoption.
...
- **Variables and attributes** describe how HDF5 datasets become table columns. Product schemas provide curated defaults for paths, element types, scalar attributes, and transforms, while users can inspect schemas, append or replace variables, or define schemas for new products with a small number of methods.
...
`SpaceAltimetry.jl` (then called SpaceLiDAR.jl) was first presented at JuliaCon 2021 [@pronkmspacelidarjlprocessingicesat22021].
...
It is currently being used for glacier modelling and student research.

# AI usage disclosure

AI tools were used during the development of parts of the software and documentation.
...
We thank the authors and maintainers of the packages in the [JuliaGeo](https://juliageo.org/) and wider Julia ecosystems on which `SpaceAltimetry.jl` builds.
